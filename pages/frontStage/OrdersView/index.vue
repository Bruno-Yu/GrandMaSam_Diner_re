<template>
  <main class="px-10 py-10 mx-auto max-w-5xl mt-20">
    <!-- progress bar -->
      <div class="max-w-4xl mx-auto my-4 pb-4">	
        <div class="flex pb-3">
          <div class="flex-1">
          </div>
        ...
        <div id="collapseOne" class="accordion-collapse collapse show " aria-labelledby="headingOne"
          ...
          <div class="accordion-body py-4 px-5">
            <ul class="border rounded p-2">
              <li v-for="(item, index) in cartProducts" :key="item.id" class="flex justify-center w-full">
      ...
      <div class="grid grid-cols-2 w-full">
        ...
        <div class="flex justify-end items-center gap-4">
          ...
          <button type="button"
            ...
            @click="confirm">{{ nextStep[currentStatus] }}</button>
        </div>
      </div>
...
const statusSteps = ['填寫訂單', '確認訂單', '成立訂單', '訂單付款']

const nextStep = ['確認', '確認', '訂單成立', '訂單完成']

export default {
  setup() {
    const { hideInfoModal, infoModal, getCart, cartProducts, finalTotal, totalQty, deleteCartProduct } = useApiModal();
    ...
    async function confirmBtn() {
      if (currentStatus.value === 0) {
        ...
        currentStatus.value += 1;
        const response = await atrApi.confirmToFormOrder();
        if (response.success) {
          // console.log(res);
          userStore.$patch((state) => { state.messageContent.message = res.message });
        } else {
