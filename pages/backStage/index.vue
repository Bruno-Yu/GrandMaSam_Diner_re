<template>
  <main>
    <div class="grid gap-2 grid-cols-1 xl:grid-cols-2 h-screen">
      <div class="flex flex-col mb-8 xl:mb-0 h-screen">
        <div class="overflow-x-auto sm:-mx-6 lg:-mx-8">
          <div class="py-4 min-w-full flex flex-col sm:px-6 lg:px-8">
            <h2 class="text-5xl font-bold pt-5 pb-8 underline decoration-4 decoration-amber-400">產品管理</h2>
            <div class="overflow-hidden border border-solid border-gray-300 mt-auto">
      ...
      <div class="flex justify-end my-3">
          ...
          <ul class="list-style-none flex gap-1">
            <li :class="{ 'hidden': !pagination.has_pre }">
              <a
                ...
                href="#"
                aria-label="Previous" @click.prevent="getAdminProducts(page = pagination.current_page - 1, category = '')">
            ...
            <li v-for="page in pagination.total_pages" :key="page">
              <a
                ...
                href="#"
                @click.prevent="getAdminProducts(page, category = '')"
                >{{ page }}</a
              >
            </li>
            <li :class="{ 'hidden': !pagination.has_next }">
              <a
                class="relative block rounded bg-transparent py-1.5 px-3 text-neutral-600 transition-all duration-300 hover:bg-black hover:text-white active:text-white dark:text-white dark:hover:bg-neutral-700 dark:hover:text-white"
                href="#"
                aria-label="Next"  @click.prevent="getAdminProducts(page = pagination.current_page + 1, category = '')"
        ...
        <p class="font-semibold">目前頁面產品共 {{ Object.keys(adminProducts).length }} 項</p>
      </div>
        ...
        <div v-if="Object.keys(currentItem).length > 0" class="flex flex-col justify-center">
          ...
          <div class="flex flex-col  rounded mb-5 bg-white shadow-lg">
            <div class="p-6 flex flex-col justify-start">
              <h5 class="text-gray-900 text-2xl font-bold mb-2 underline decoration-4 decoration-amber-400">{{ currentItem.title }}</h5>
              ...
              <p><span class="font-bold ">售價: </span> <span class="underline">{{ currentItem.price }}/{{ currentItem.unit }}</span> <span
                  class="text-gray-600 ml-5 text-xs">還剩  {{ currentItem.num }} 份</span> </p>
                </div>
              </div>
              <div v-if="currentItem.imagesUrl?.length" class="grid gap-1 grid-rows-1 grid-cols-5 p-1 rounded bg-black shadow-lg">
